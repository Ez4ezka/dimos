...
--json`` for direct-or-relayed, ``ip route`` for the interface that actually carries the
traffic to the ground station (the modem's signal only counts when that is the modem),
``ping`` and that interface's counters. Parsers are pure; the hardware sources degrade to
"not present", never raise. The replay sources carry the scenarios measured on
2026-09-08/09 and the 2026-09-17 bench so the policy is tested and gated without a modem.
"""
...
class ModemReading:
    ...
    band: str = ""
    cell_id: str = ""
    raw: str = ""  # the +QCSQ line, kept when its numbers are not believable
...
# What a real radio reports. A line outside these is a layout this parser has not seen
# (the bench reading of 2026-09-17 had an RSRP of +31): it is reported, never trusted.
_RSRP_DBM = (-160.0, -40.0)
_SINR_DB = (-30.0, 50.0)
_RSRQ_DB = (-45.0, 0.0)


def _believable(rsrp: float, sinr: float, rsrq: float) -> bool:
    return (
        _RSRP_DBM[0] <= rsrp <= _RSRP_DBM[1]
        and _SINR_DB[0] <= sinr <= _SINR_DB[1]
        and (math.isnan(rsrq) or _RSRQ_DB[0] <= rsrq <= _RSRQ_DB[1])
    )
...
def parse_qcsq(reply: str) -> ModemReading:
    ...
    if mode in ("NOSERVICE", "") or len(parts) < 2:
        return ModemReading(present=True, sysmode=mode, raw=raw)
    values = [_num(v) for v in parts[1:]] + [math.nan] * 3
    if mode == "NR5G-SA":
        rsrp, sinr, rsrq = values[0], values[1], values[2]  # <RSRP>,<SINR>,<RSRQ>
    elif mode == "NR5G-NSA" and len(parts) >= 8:
        rsrp, sinr, rsrq = values[4], values[5], values[6]  # LTE quartet, then the NR trio
    else:
        rsrp, sinr, rsrq = values[1], values[2], values[3]  # LTE: <RSSI>,<RSRP>,<SINR>,<RSRQ>
    if not _believable(rsrp, sinr, rsrq):
        return ModemReading(present=True, sysmode=mode, raw=raw)
    return ModemReading(present=True, sysmode=mode, rsrp_dbm=rsrp, sinr_db=sinr, rsrq_db=rsrq)
...
def parse_route(output: str) -> str:
    """The egress interface in ``ip -j route`` output; empty when there is none."""
    try:
...
def _host(address: str) -> str:
    """``10.0.0.188:41641`` or ``[fd7a::1]:41641`` -> the host part."""
    return address.rsplit(":", 1)[0].strip("[]") if ":" in address else address
...
class RouteSource(Protocol):
    def read(self, host: str) -> str: ...


class CounterSource(Protocol):
    def read(self, interface: str) -> IfaceCounters | None: ...
...
class KernelRoute:
    """The interface the kernel sends packets for ``host`` on; the default route when unset."""

    def read(self, host: str) -> str:
        cmd = (
            ["ip", "-j", "route", "get", host] if host else ["ip", "-j", "route", "show", "default"]
        )
        try:
            out = subprocess.run(cmd, capture_output=True, text=True, timeout=2.0, check=False)
        except (OSError, subprocess.TimeoutExpired):
            return ""
        return parse_route(out.stdout) if out.returncode == 0 else ""
...
class SysfsCounters:
    def read(self, interface: str) -> IfaceCounters | None:
        ...
        try:
            ...
            return None
        return IfaceCounters(interface, tx, rx, time.time())
...
class Scenario:
    ...
    ping: str
    interface: str  # what the kernel routes the ground station over
    tx_bps: float
    expect_video: bool
...
def _ts(direct: bool, online: bool = True) -> dict[str, Any]:
    return {
        "Self": {"HostName": "aircraft", "TailscaleIPs": ["100.64.0.1"]},
        "Peer": {
            "gs": {
...
SCENARIOS: dict[str, Scenario] = {
    ...
    "bench_wifi_poor_cell": Scenario(
        name="bench_wifi_poor_cell",
        qcsq='+QCSQ: "NR5G-SA",-104,-10,-14\r\n\r\nOK\r\n',
        qeng='+QENG: "servingcell","NOCONN","NR5G-SA","TDD",310,260,1C3C58,17,7F01,520110,41,12,-104,-14,-10,1,-\r\n\r\nOK\r\n',
        tailscale=_ts(direct=True),
        ping=_ping(3.0, 0.0),
        interface="wlP1p1s0",
        tx_bps=50_000.0,
        expect_video=True,
        expect_telemetry="full",
        note="2026-09-17 bench: a poor cell, but the laptop is reached over the LAN, so video",
    ),
...
class ReplayCounters:
    ...
    def read(self, interface: str) -> IfaceCounters | None:
        ...
        return IfaceCounters(interface, tx, tx // 4, now)
...
class LinkMonitorConfig(ModuleConfig):
    ...
    # Site configuration, e.g. Peer(name="ground-station", address="100.64.0.2", role="ground").
    peers: tuple[Peer, ...] = Field(default=())
    # Which peer the round trip and throughput are measured against: a name in ``peers``,
    # or a hostname or address to use directly.
    ground_station: str = Field(default="ground-station")
...
def estimate_uplink_bps(
    modem: ModemReading, path: PathReading, ping: PingReading, cellular: bool
) -> float:
    ...
    relay: 0.26 Mbit/s spare). The signal only counts when the ground station is reached
    through the modem (``cellular``). NaN when nothing is known."""
    if not modem.present and not ping.reachable:
        return math.nan
    if not cellular:
        base = 2_000_000.0  # Wi-Fi or wired: not the bottleneck at these rates
    elif math.isnan(modem.sinr_db):
        base = 250_000.0  # a cell whose signal could not be read: assume the relay class
    else:
        sinr = modem.sinr_db
...
class LinkMonitor(Module):
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        self._route: RouteSource | None = None
    ...
    def setup_sources(self) -> None:
        ...
        if cfg.source == "replay":
            ...
            self._route = ReplayRoute(scenario)
            ...
            self._route = KernelRoute()
            self._counters = SysfsCounters()
    ...
    def poll(self, now: float | None = None) -> tuple[LinkStatus, LinkPolicy]:
        ...
        assert self._modem and self._path and self._ping and self._route and self._counters
        wall = time.time() if now is None else now
        modem = self._modem.read()
        path = self._path.read()
        ping = self._ping.read()
        # A direct overlay path has a real endpoint to route to; a relayed one leaves by
        # the default route.
        egress = self._route.read(_host(path.address) if path.direct else "")
        counters = self._counters.read(egress)
        up_bps, down_bps = self._throughput(counters)
        status = self._status(modem, path, ping, egress, up_bps, down_bps, wall)
    ...
    def _status(
        ...
        cfg = self.config
        cellular = egress == cfg.interface if egress else modem.present
        usable = estimate_uplink_bps(modem, path, ping, cellular)
        if self._warming_up:
            usable = math.nan
        ...
        if modem.present and math.isnan(modem.sinr_db) and modem.raw:
            notes.append(f"modem signal not believable: {modem.raw}")
        return LinkStatus(
