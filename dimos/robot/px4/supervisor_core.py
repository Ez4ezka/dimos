...
Ported from drone-autonomy ``flight_supervisor.py:43-303`` (flown 2026-09-09). Additions
over the flown code: the TELEOP guidance mode, the E-STOP latch, the closed rejection
enum, and ``set_hold``/``set_land`` gated on PX4 being in OFFBOARD (safety invariant 1).
"""
...
class SupervisorCore:
    ...
    def step(self, st: VehicleSnapshot, m: Px4Actuator, now: float) -> None:
        ...
        if s == "IDLE":
            ...
            if in_offboard:
                ...
                self.goto(
                    "IDLE",
                    "PX4 did not enter OFFBOARD (check COM_RC_OVERRIDE / preflight in QGC)",
                    now,
                )
        elif s == "ARMING":
            if armed:
                ...
                m.set_mode(MAIN_AUTO, SUB_AUTO_LOITER)
                self.goto("IDLE", "arming refused (see QGC messages)", now)
        elif s == "TAKEOFF":
    ...
    def status(self, st: VehicleSnapshot, now: float | None = None) -> dict[str, Any]:
        """``supervisor_status_v1``, field-compatible with the flown packet (``opcmd.py watch``)."""
        t = time.time() if now is None else now
        hb = st.heartbeat
        sp = self.sp
        out: dict[str, Any] = dict(
            schema="supervisor_status_v1",
            t=t,
            state=self.state,
            reason=self.reason,
