...
_PEERS = (
    {"name": "aircraft", "address": "100.64.0.1", "role": "aircraft"},
    {"name": "ground-station", "address": "100.64.0.2", "role": "ground"},
)


def _monitor(scenario: str) -> tuple[LinkMonitor, dict[str, list[Any]]]:
    m = LinkMonitor(source="replay", replay_scenario=scenario, peers=_PEERS)
...
def test_parsers_read_the_quectel_and_tailscale_formats() -> None:
    ...
    path = parse_tailscale_status(SCENARIOS["downtown_idle"].tailscale, "ground-station")
    assert (path.available, path.online, path.direct, path.relay) == (True, True, False, "sfo")
    assert parse_tailscale_status(SCENARIOS["home_5g"].tailscale, "100.64.0.2").direct
    ...
    assert parse_route('[{"dst":"10.0.0.1","gateway":"10.0.0.1","dev":"wlP1p1s0"}]') == "wlP1p1s0"
    assert parse_route("[]") == "" and parse_route("not json") == ""


def test_unbelievable_signal_is_reported_not_trusted() -> None:
    # The 2026-09-17 bench reply: two numbers, the first a positive "RSRP".
    r = parse_qcsq('+QCSQ: "NR5G-SA",31,-10\r\n\r\nOK\r\n')
    assert r.present and math.isnan(r.rsrp_dbm) and math.isnan(r.sinr_db)
    assert r.raw == '+QCSQ: "NR5G-SA",31,-10'
    assert parse_qcsq('+QCSQ: "NR5G-SA",-104,-10,-14').sinr_db == -10.0
...
def test_uplink_estimate_orders_the_measured_scenarios() -> None:
    def est(name: str) -> float:
        ...
        return estimate_uplink_bps(
            ...
            parse_tailscale_status(s.tailscale, "ground-station")
            if s.tailscale
            else PathReading(False),
            parse_ping(s.ping) if s.ping else PingReading(False),
            cellular=s.interface == "wwan5g",
        )
    ...
    assert est("bench_wifi_poor_cell") >= 1_000_000.0  # SINR -10, but the path is the LAN
...
def test_poor_cell_does_not_block_video_on_the_lan() -> None:
    m, _ = _monitor("bench_wifi_poor_cell")
    try:
        m.poll(now=999.0)
        status, policy = m.poll(now=1000.0)
        assert status.interface == "wlP1p1s0" and status.sinr_db == -10.0
        assert status.usable_uplink_bps > 1_000_000.0
        assert policy.video_allowed and policy.telemetry_profile == "full"
    finally:
        m.stop()


def test_a_bad_signal_reading_is_conservative_and_noted() -> None:
    bad = ModemReading(present=True, sysmode="NR5G-SA", raw='+QCSQ: "NR5G-SA",31,-10')
    path, ping = PathReading(True, online=True, direct=True), PingReading(True, 100.0, 0.0)
    assert estimate_uplink_bps(bad, path, ping, cellular=True) == 250_000.0  # relay class
    assert estimate_uplink_bps(bad, path, ping, cellular=False) == 2_000_000.0
    m, _ = _monitor("home_5g")
    try:
        m._modem = type("M", (), {"read": staticmethod(lambda: bad)})()
        m.poll(now=999.0)
        status, _ = m.poll(now=1000.0)
        assert "not believable: +QCSQ" in status.notes
    finally:
        m.stop()
